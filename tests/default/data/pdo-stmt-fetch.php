...
class Foo
{
    public function fetchAll(PDO $pdo)
    {
        ...
        $all = $stmt->fetchAll();
        assertType('array<int, array{email: string, 0: string, adaid: int<-32768, 32767>, 1: int<-32768, 32767>}>', $all);

        $all = $stmt->fetchAll(PDO::FETCH_BOTH);
        assertType('array<int, array{email: string, 0: string, adaid: int<-32768, 32767>, 1: int<-32768, 32767>}>', $all);

        $all = $stmt->fetchAll(PDO::FETCH_NUM);
        assertType('array<int, array{string, int<-32768, 32767>}>', $all);

        $all = $stmt->fetchAll(PDO::FETCH_ASSOC);
        assertType('array<int, array{email: string, adaid: int<-32768, 32767>}>', $all);
        ...
        assertType('array<int, int<-32768, 32767>>', $all);

        $all = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
        assertType('array<string, int<-32768, 32767>>', $all);
    ...
    public function fetch(PDO $pdo)
    {
        ...
        // default fetch-mode is BOTH
        $all = $stmt->fetch();
        assertType('array{email: string, 0: string, adaid: int<-32768, 32767>, 1: int<-32768, 32767>}|false', $all);

        $all = $stmt->fetch(PDO::FETCH_BOTH);
        assertType('array{email: string, 0: string, adaid: int<-32768, 32767>, 1: int<-32768, 32767>}|false', $all);

        $all = $stmt->fetch(PDO::FETCH_NUM);
        assertType('array{string, int<-32768, 32767>}|false', $all);
        ...
        assertType('int<-32768, 32767>|false', $all);
